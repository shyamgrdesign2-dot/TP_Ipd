...
import { addUpdateVitals, getVitals } from "../../redux/vitalsSlice";
import moment from "moment";

const dateFormat = "YYYY-MM-DD";
const showDateFormat = "DD MMM, YY";

function AddMeasurements(props) {
  const scrollContainerRef = useRef(null);
  const inputRef = useRef([]);

  const { handleMeasurementsDrawer, handleCollapsed } = props;

  const { selectedVitalsList, loading } = useSelector((state) => state.vitals);
  const dispatch = useDispatch();

  const { patient_data, vitalsData, setVitalsData } =
  ...
  useEffect(() => {
    if (scrollContainerRef.current) {
      ...
      const data = childVitalsData.sort(
  ...
  const calculate = (H, W) => {
    ...
    var age =
      patient_data !== undefined && patient_data.ageYears !== undefined
            ...
            <div key={i} className="vitals-wrap-body w-100 vitals-child-width">
              ...
              <div className="vitals-row vitals-row-60 d-flex align-items-center px-2 w-100">
                <Input
                  ...
                  value={item.height}
                  addonAfter={"cms"}
                  onChange={(e) => onChangeInput(e.target.value, i, 7)}
                />
              </div>
              <div className="vitals-row vitals-row-60 d-flex align-items-center px-2 w-100">
                <Input
                  ...
                  value={item.weight}
                  addonAfter={"kgs"}
                  onChange={(e) => onChangeInput(e.target.value, i, 8)}
                />
              </div>
              ...
              <div className="vitals-row vitals-row-60 d-flex align-items-center px-2 w-100">
                <Input
                  ...
                  value={item.ofc}
                  addonAfter={"cms"}
                  onChange={(e) => onChangeInput(e.target.value, i, 9)}
                />
              </div>
            </div>
          );
        ...
        <div className="modalCard-header h-60 align-items-center justify-content-between d-flex">
          <div className="align-items-center d-flex">
            <Button
              ...
              onClick={handleMeasurementsDrawer}
            >
          ...
          <Button
            ...
            loading={loading}
            disabled={childVitalsData.length > 0 ? false : true}
          >
            Done
          </Button>
        </div>
...
export default React.memo(AddMeasurements);
