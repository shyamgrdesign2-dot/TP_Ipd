...
const ObstetricSummary = ({ data, pastPregnancyData }) => {
  ...
  const patientInfoSection = {
    ...
    values: [[
      ...
      data?.gestationWeeks || data?.gestationDays ? 
        `${data?.gestationWeeks ? `${data?.gestationWeeks}W` : ''}${data?.gestationWeeks && data?.gestationDays ? ', ' : ''}${data?.gestationDays ? `${data?.gestationDays}D` : ''}` 
        : '-',
      data?.blood || '-',
      data?.husbandsBlood || '-',
  ...
  const gplaeSection = {
    columns: ['Gravida', 'Para', 'Living', 'Abortion', 'NND', 'Ectopic'],
    values: [[
      data?.gravidity || '0',
      data?.parity || '0',
  ...
  const pregnancyHistorySection = {
    ...
    values: pastPregnancyData?.map((pregnancy, index) => [
      (index + 1).toString(),
      pregnancy.outcome || '-',
  ...
  const examinationSection = {
    ...
    values: data?.examinationHistory?.map(exam => [
      formatDateToShortMonthYear(exam.date),
      exam.pallor ? 'Yes' : 'No',
      exam.oedema ? 'Yes' : 'No',
      exam.mothersBMI ? `${exam.mothersBMI} Kg/m2` : '-',
      exam.systolic && exam.diastolic ? `${exam.systolic}/${exam.diastolic}mg` : '-',
      exam.heightOfFundus ? `${exam.heightOfFundus} ${exam.heightOfFundusUnit}` : '-',
      exam.presentation || '-',
      exam.liquor || '-',
      exam?.foetalHeartRate ? `${exam?.foetalHeartRate}bpm` : '-'
    ]) || [],
    remarks: data?.examinationHistory?.[0]?.notes || 'No examination notes available'
  };

  const ancSchedulerSection = {
    columns: ['Test Name', 'Due Date', 'Status', 'Remark'],
    values: data?.ancHistory?.filter(test => test.dueDate || test.status !== 'Due')?.map(test => [
      test.master.name,
  ...
  const immunisationSection = {
    ...
    values: data?.immunisationHistory?.map(vaccine => [
      vaccine.master.name,
      vaccine.givenDate ? `Given on ${formatDateToShortMonthYear(vaccine.givenDate)}` : vaccine.status,
      vaccine.notes || '-'
    ]) || []
  };
  ...
  const hasValidData = (section) => {
    ...
    if (section.values.length === 1) {
      const hasNonEmptyValue = section.values[0].some(value => 
        value !== '-' && value !== '0' && value !== ''
      );
  ...
  const sectionsToShow = [
    hasValidData(patientInfoSection) && { ...patientInfoSection, title: 'Patient Information' },
    hasValidData(gplaeSection) && { ...gplaeSection, title: 'GPLAE' },
    hasValidData(pregnancyHistorySection) && { ...pregnancyHistorySection, title: 'Pregnancy history' },
    hasValidData(examinationSection) && { ...examinationSection, title: 'Examination' },
    hasValidData(ancSchedulerSection) && { ...ancSchedulerSection, title: 'ANC Scheduler' },
    hasValidData(immunisationSection) && { ...immunisationSection, title: 'Immunisation Vaccine' }
  ].filter(Boolean);
