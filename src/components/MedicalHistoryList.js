...
function MedicalHistoryList(props) {
  const {
    gynecHistory: gynecHistoryFromProps,
  ...
  const { medicalHistoryData, gynecHistoryData: gynecHistoryFromStore } =
  ...
  const filteredGynecHistory = Object.keys(gynecHistory || {}).reduce(
    (acc, key) => {
    ...
    if (medicalHistoryData.length > 0) {
                          ...
                          <>
                              ...
                              {(() => {
                                ...
                                if (e?.tmmhs_id != 3) {
                                  if (e1?.status)
                                    parts.push(
                                      <>
                                        <b>Status</b>: {e1.status}
                                      </>
                                    );
                                  if (e1?.medication)
                                    parts.push(
                                      <>
                                        <b>Medication</b>: {e1.medication}
                                      </>
                                    );
                                }
                                if (e?.tmmhs_id == 3 && e1?.relationship) {
                                  parts.push(
                                    <>
                                      <b>Relationship</b>: {e1.relationship}
                                    </>
                                  );
                                }
                                if (e1?.note)
                            ...
                            {e1?.medical_history_remarks && (
                              <li key={Math.random()} className="my-2">
                                ...
                                <label>{e1?.medical_history_remarks}</label>
                              </li>
                            )}
                          </>
                        );
      ...
      if (medicalHistoryData?.[0]?.medical_history_remarks) {
        data.push({
          ...
          label: (
            <ul style={{ margin: 0 }}>
              <li className="fw-semibold">Additional History</li>
            </ul>
          ),
          children: (
            <ul style={{ paddingLeft: "48px", paddingRight: "48px" }}>
              <li className="fontroboto p-1 text-history fw-normal">
  ...
  return (
    <>
      <div
        className={`ipd-mhl-container ${
          isDischargeSummary
            ? "ipd-mhl-container-discharge-summary"
            : "overflow-y-auto"
        }`}
        // style={isDischargeSummary ? {} : { maxHeight: "661px" }}
      >
        {(medicalHistoryData.length > 0 ||
          ...
          <div className="p-10">
            {isGynaecHistoryAccessable &&
              gynecHistory &&
              Object.keys(filteredGynecHistory).length > 0 && (
