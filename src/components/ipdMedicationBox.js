...
function MedicationsBox(props) {
  const { isEditable = true, isView = false } = props;
  ...
  const TABLE_MEDICATION = useMemo(() => {
    const rows = (medicationData || []).map((item, index) => ({
      ...
      unitPerDose: item?.tmm_dosage_unit_name || "--",
      frequency: item?.tmm_freq_type_name || "--",
    ...
    const handleEditClick = (item) => {
      try {
        ...
        setAddCustom && setAddCustom(updateItem);
      } catch (e) {
        console.error("Failed to trigger edit for medication", e);
      }
    };
              ...
              <Droppable droppableId="medication" direction="vertical">
                {(provided) => (
                  <tbody ref={provided.innerRef} {...provided.droppableProps}>
                    {rows.map((row, idx) => (
                      <Draggable key={`medication-${idx}`} draggableId={`medication-${idx}`} index={idx}>
                        {(provided) => (
                          <tr ref={provided.innerRef} {...provided.draggableProps}>
                            <td style={{textAlign: 'center'}}>
                              <MenuOutlined {...provided.dragHandleProps} className="drag-handle" style={{ cursor: 'grab' }} />
                            </td>
                            <td>
                              <div className="fontroboto fw-medium">{row.medicineName}</div>
                              {row.genericName ? (
                                <div className="text-truncate fw-normal me-1">{row.genericName}</div>
                              ) : null}
                            </td>
                            <td>{row.unitPerDose}</td>
                            <td>{row.frequency}</td>
                            <td>{row.when}</td>
                            <td>{row.duration}</td>
                            <td>{row.note}</td>
                            <td>
                              {(!row.originalItem?.pms_default && isEditable) ? (
                                <div className="d-flex align-items-center gap-3">
                                  <i
                                    className="icon-Edit fs-18"
                                    onClick={() => handleEditClick(row.originalItem)}
                                    style={{ cursor: 'pointer' }}
                                  ></i>
                                  <i
                                    className="icon-delete fs-18"
                                    onClick={() => onRemoveRow && onRemoveRow(row.index)}
                                    style={{ cursor: 'pointer' }}
                                  ></i>
                                </div>
                              ) : (
  ...
  }, [medicationData, isEditable, medicineTypeList]);
